...
- **Dashboard API Integration**: The React Dashboard uses `src/services/apiClient.js` to fetch live incident and takedown data from the Render backend.
- **Auth Handshake (Dashboard to Extension)**: On login, the dashboard emits a `SAFELENS_AUTH_INIT` message via `window.postMessage`. The `dashboardBridge.js` content script relays it to the background Service Worker, which stores the token locally. `bridgeClient.js` then attaches this token as a `Bearer` header for all telemetry (`LOG_SCAN` and `REGISTER_BACKEND_ASSET`).
- **Bidirectional Actions & Optimistic Sync**: Dashboard actions (`updateIncidentStatus`, `triggerLegalTakedown`) implement the **Optimistic UI** pattern (state updates immediately, reverts to original state on API failure) with floating visual Toast feedback. Successful actions trigger background fetches to force synchronicity with the backend.
