...
async function Me(e) {
  ...
  try {
    ...
    for (let C = 0; C < o.rows; ++C) {
      const v = o.data32S[C * 4], S = o.data32S[C * 4 + 1], P = o.data32S[C * 4 + 2], O = o.data32S[C * 4 + 3], D = Math.atan2(O - S, P - v) * (180 / Math.PI);
      D > -45 && D < 45 && (g += D, p++);
    }
    if (p < 3)
...
(function(e) {
  var t = function(n) {
    ...
    var T = O.prototype = S.prototype = Object.create(D);
    P.prototype = O, a(T, "constructor", { value: O, configurable: !0 }), a(
      O,
      "constructor",
    ...
    n.isGeneratorFunction = function(l) {
      ...
      return Object.setPrototypeOf ? Object.setPrototypeOf(l, O) : (l.__proto__ = O, s(l, p, "GeneratorFunction")), l.prototype = Object.create(T), l;
    }, n.awrap = function(l) {
      return { __await: l };
    };
    function j(l, c) {
    ...
    L(T), s(T, p, "Generator"), s(T, d, function() {
      return this;
...
var De = ({
  ...
  let r = e;
  return typeof r > "u" && (r = Te("Job", ue), ue += 1), {
    id: r,
    action: t,
    payload: n
  };
...
var Ve = ce;
const Je = Ue, Ke = He, Xe = Ye, Qe = je, et = Ze, tt = Ve;
var rt = {
  defaultOptions: Je,
  spawnWorker: Ke,
  terminateWorker: Xe,
...
var ve = async (e = "eng", t = Y.LSTM_ONLY, n = {}, r = {}) => {
  ...
  const v = ({ id: x, action: b, payload: l }) => new Promise((c, f) => {
    ...
    return g.push(...I), I.length > 0 ? T(I, c).then(() => L(x, f, y, c)) : L(x, f, y, c);
  }, re = (x = {}, b) => v(W({
    id: b,
  ...
  return P().then(() => T(e)).then(() => L(e, t, r)).then(() => A(V)).catch(() => {
  }), k;
};
...
async function Ht(e, t) {
  try {
    ...
    const n = e.getContext("2d"), r = e.width, o = e.height, a = n.getImageData(0, 0, r, o), i = a.data, u = Ut(t + "\0");
    let d = 0;
    ...
    for (let s = 0; s < p; s += $)
      for (let h = 0; h < g; h += $) {
        ...
        for (let v = 0; v < $; v++)
          for (let S = 0; S < $; S++) {
            const P = ((s + v) * r + (h + S)) * 4, O = i[P], D = i[P + 1], B = i[P + 2];
            m[v][S] = 0.299 * O + 0.587 * D + 0.114 * B, A[v][S] = 128 - 0.1687 * O - 0.3313 * D + 0.5 * B, k[v][S] = 128 + 0.5 * O - 0.4187 * D - 0.0813 * B;
          }
        const E = Wt(m);
        if (d < u.length) {
        ...
        for (let v = 0; v < $; v++)
          for (let S = 0; S < $; S++) {
            const P = ((s + v) * r + (h + S)) * 4, O = C[v][S], D = A[v][S], B = k[v][S];
            let R = Math.round(O + 1.402 * (B - 128)), T = Math.round(O - 0.3441 * (D - 128) - 0.7141 * (B - 128)), L = Math.round(O + 1.772 * (D - 128));
            i[P] = Math.max(0, Math.min(255, R)), i[P + 1] = Math.max(0, Math.min(255, T)), i[P + 2] = Math.max(0, Math.min(255, L));
          }
      }
...
async function Ie(e, t, n = 15) {
  ...
  try {
    t.forEach((o) => {
      ...
      m.width = s, m.height = h, m.getContext("2d").drawImage(e, g, p, s, h, 0, 0, s, h), r.save();
      try {
        r.beginPath(), r.rect(g, p, s, h), r.clip(), r.filter = `blur(${n}px)`, r.drawImage(m, g, p);
      } finally {
        r.restore();
      }
    });
    ...
    for (let k = 0; k < h; k += n)
      for (let E = 0; E < s; E += n) {
        ...
        for (let R = 0; R < n && k + R < h; R++)
          for (let T = 0; T < n && E + T < s; T++) {
            const L = ((k + R) * s + (E + T)) * 4;
            C += A[L], v += A[L + 1], S += A[L + 2], P++;
          }
        const O = Math.round(C / P), D = Math.round(v / P), B = Math.round(S / P);
        for (let R = 0; R < n && k + R < h; R++)
          for (let T = 0; T < n && E + T < s; T++) {
            const L = ((k + R) * s + (E + T)) * 4;
            A[L] = O, A[L + 1] = D, A[L + 2] = B;
          }
      }
...
async function Qt(e, t, n = "redact") {
  return console.log(`[AIService] Delegating redaction request (mode: ${n}) for ${t.length} regions.`), n === "blur" ? Ie(e, t, 8) : Vt(e, t, "redact", { fillStyle: "#000000" });
}
